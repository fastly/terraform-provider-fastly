...
- Sensitive credential fields (access keys, secrets, tokens, IAM roles, etc.) belong in a nested `SingleNestedAttribute` block (e.g. `authentication`), never as flat top-level attributes — even when there is only a single sensitive field. See the example `authentication` in `internal/resources/loggings3/schema.go`  
