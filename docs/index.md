...
* `no_auth` - (Optional) Set to `true` only if your configuration consumes data sources that do not require authentication, such as `fastly_ip_ranges`. Default: `false`
...
- **no_auth** (Boolean) Set to `true` only if your configuration consumes data sources that do not require authentication, such as `fastly_ip_ranges`
