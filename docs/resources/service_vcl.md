...
- `max_lifetime` (Number) Maximum time from creation (in milliseconds) that a pooled HTTP keepalive connection will be eligible for reuse; 0 is treated as unlimited.
- `max_tls_version` (String) Maximum allowed TLS version on SSL connections to this backend.
- `max_use` (Number) Maximum number of requests allowed over a single, pooled HTTP keepalive connection to this backend; 0 is treated as unlimited.
