...
3. The provider reconciles the nested `domain`, `backend`, `acl`, `condition`,
   `healthcheck`, and `gzip` blocks to version `1`. `condition` is reconciled
   before `backend` and `gzip` since both can reference a condition by name,
   and `healthcheck` is reconciled before `backend` since a backend can
   reference a health check by name - in both cases the Fastly API rejects a
   config that names a condition or health check which doesn't exist yet in
   that version.
