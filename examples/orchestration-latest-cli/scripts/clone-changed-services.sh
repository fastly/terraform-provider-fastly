...
if [[ -z "${changed_service_ids}" ]]; then
  echo "No changed Fastly services found in plan; nothing to clone."
  exit 0
fi
...
while read -r service_id; do
  ...
  if [[ "${service_id}" == "${service_1_id}" ]]; then
    echo "Cloning changed service: service-1 (${service_id}) version latest"
    ./scripts/clone.sh service-1 latest
  elif [[ "${service_id}" == "${service_2_id}" ]]; then
    echo "Cloning changed service: service-2 (${service_id}) version latest"
