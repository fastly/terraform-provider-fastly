...
# Service 1 configuration targets the latest version number returned by the
# data source. After cloning a new draft, a normal terraform apply will pick up
# that latest version automatically.
module "service_1" {
...
# Service 2 configuration also targets the latest version number returned by
# the data source.
module "service_2" {
  source          = "./modules/service_config"
  service_id      = fastly_service.service_2.id
