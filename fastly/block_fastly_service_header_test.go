	...
	cases := []struct {
	...
	}{
		{
			remote: []*gofastly.Header{
				{
					Name:              new("myheader"),
					Action:            new(gofastly.HeaderActionDelete),
					IgnoreIfSet:       new(true),
					Type:              new(gofastly.HeaderTypeCache),
					Destination:       new("http.aws-id"),
					Source:            new(""),
					Regex:             new(""),
					Substitution:      new(""),
					Priority:          new(100),
...
func TestFastlyServiceVCL_BuildHeaders(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			remote: &gofastly.CreateHeaderInput{
				Action:            new(gofastly.HeaderActionDelete),
				CacheCondition:    new("test"),
				Destination:       new("http.aws-id"),
				IgnoreIfSet:       new(gofastly.Compatibool(true)),
				Name:              new("someheadder"),
				Priority:          new(100),
				Regex:             new("test"),
				RequestCondition:  new("test"),
				ResponseCondition: new("test"),
				Source:            new("test"),
				Substitution:      new("test"),
				Type:              new(gofastly.HeaderTypeCache),
			},
			local: map[string]any{
				"action":             "delete",
		...
		{
			remote: &gofastly.CreateHeaderInput{
				Action:            new(gofastly.HeaderActionSet),
				CacheCondition:    new(""),
				Destination:       new("http.aws-id"),
				IgnoreIfSet:       new(gofastly.Compatibool(false)),
				Name:              new("someheadder"),
				Priority:          new(100),
				Regex:             new(""),
				RequestCondition:  new(""),
...
func TestAccFastlyServiceVCL_headers_basic(t *testing.T) {
	...
	log1 := gofastly.Header{
		Action:            new(gofastly.HeaderActionDelete),
		CacheCondition:    new(""),
		Destination:       new("http.x-amz-request-id"),
		IgnoreIfSet:       new(false),
		Name:              new("remove x-amz-request-id"),
		Priority:          new(100),
		Regex:             new(""),
		RequestCondition:  new(""),
		ResponseCondition: new(""),
	...
	log2 := gofastly.Header{
		Action:            new(gofastly.HeaderActionDelete),
		CacheCondition:    new(""),
		Destination:       new("http.Server"),
		IgnoreIfSet:       new(true),
		Name:              new("remove s3 server"),
		Priority:          new(100),
		Regex:             new(""),
		RequestCondition:  new(""),
		ResponseCondition: new(""),
	...
	log4 := gofastly.Header{
		Action:            new(gofastly.HeaderActionSet),
		CacheCondition:    new("test_cache_condition"),
		Destination:       new("http.server-name"),
		IgnoreIfSet:       new(false),
		Name:              new("Add server name"),
		Priority:          new(100),
		Regex:             new(""),
		RequestCondition:  new("test_req_condition"),
		ResponseCondition: new("test_res_condition"),
		ServiceVersion:    new(1),
		Source:            new("server.identity"),
		Substitution:      new(""),
		Type:              new(gofastly.HeaderTypeRequest),
	}
