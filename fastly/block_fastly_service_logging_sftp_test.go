...
func TestAccLoggingSFTP_vcl_basic(t *testing.T) {
...
func TestAccLoggingSFTP_compute_basic(t *testing.T) {
	var service gofastly.ServiceDetail
	name := fmt.Sprintf("tf-test-%s", acctest.RandString(10))
	domain := fmt.Sprintf("fastly-test.%s.com", name)
...
func TestAccLoggingSFTP_password_secret_vcl_key(t *testing.T) {
	name := fmt.Sprintf("tf-test-%s", acctest.RandString(10))
	domain := fmt.Sprintf("fastly-test.%s.com", name)
