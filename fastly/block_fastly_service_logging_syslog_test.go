...
func TestAccLoggingSyslog_vcl_basic(t *testing.T) {
...
func TestAccLoggingSyslog_compute_basic(t *testing.T) {
	var service gofastly.ServiceDetail
	name := fmt.Sprintf("tf-test-%s", acctest.RandString(10))
	domainName1 := fmt.Sprintf("fastly-test.tf-%s.com", acctest.RandString(10))
...
func TestAccLoggingSyslog_vcl_useTLS(t *testing.T) {
...
func testAccCheckFastlyServiceVCLSyslogAttributes(service *gofastly.ServiceDetail, syslogs []*gofastly.Syslog, serviceType string) resource.TestCheckFunc {
	return func(_ *terraform.State) error {
		...
		syslogList, err := conn.ListSyslogs(context.TODO(), &gofastly.ListSyslogsInput{
			...
			for _, ls := range syslogList {
				if gofastly.ToValue(s.Name) == gofastly.ToValue(ls.Name) {
					...
					if diff := cmp.Diff(s, ls); diff != "" {
						return fmt.Errorf("bad match Syslog logging match: %s", diff)
					}
					found++
				}
			}
			...
			local: []map[string]any{
				{
					...
					"address":            "127.0.0.1",
					"port":               8080,
					"format":             LoggingSyslogDefaultFormat,
					"format_version":     1,
					"response_condition": "response_condition_test",
