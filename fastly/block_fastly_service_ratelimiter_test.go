...
func TestResourceFastlyFlattenRateLimiter(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			remote: []*gofastly.ERL{
				{
					Action: new(gofastly.ERLActionResponse),
					ClientKey: []*string{
						new("req.http.Fastly-Client-IP"),
						new("req.http.User-Agent"),
					},
					...
					RateLimiterID:      new("123abc"),
					LoggerType:         new(gofastly.ERLLogBigQuery),
					Name:               new("example"),
					PenaltyBoxDuration: new(123),
					Response: &gofastly.ERLResponse{
						ERLContent:     new("example"),
						ERLContentType: new("plain/text"),
						ERLStatus:      new(429),
					},
					ResponseObjectName: new("example"),
					RpsLimit:           new(123),
					WindowSize:         new(gofastly.ERLSize1),
				},
			},
			local: []map[string]any{
				{
...
func TestAccFastlyServiceVCLRateLimiter_basic(t *testing.T) {
	...
	erl1 := gofastly.ERL{
		...
		RpsLimit:   new(100),
		WindowSize: new(gofastly.ERLSize60),
	}
	...
	erl1Updated := gofastly.ERL{
		...
		RpsLimit:          new(100),
		WindowSize:        new(gofastly.ERLSize60),
		URIDictionaryName: new("rate_limit_endpoints"),
	}
