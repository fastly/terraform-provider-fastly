...
// LoggingBlobStorageDefaultFormat is the default log format for Blob Storage
// logging. Identical body to LoggingS3DefaultFormat — verified against
// main:fastly/constants.go, where the two are also byte-identical.
...
// copied byte-for-byte from main:fastly/constants.go. Unlike the other
// constants in this file, this one has not been verified against a live
// endpoint via the Compute-path *_formatDefault acceptance test (see
// TestAccFastlyServiceLoggingSplunk_formatDefault) — run it with
// FASTLY_API_TOKEN before trusting this value if the API's behavior changes.
