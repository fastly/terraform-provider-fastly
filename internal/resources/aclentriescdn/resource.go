...
func (r *Resource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	plan.Entry = flattenEntries(ctx, refreshedEntries, &resp.Diagnostics)
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}
...
func (r *Resource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	state.Entry = flattenEntries(ctx, remoteState, &resp.Diagnostics)
...
func (r *Resource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	oldByContent := make(map[string]EntryModel)
	for _, e := range oldEntries {
		if !e.ID.IsNull() && !e.ID.IsUnknown() {
			key := plannedEntryContentKey(e)
			oldByContent[key] = e
		}
	}

	newByContent := make(map[string]EntryModel)
	for _, e := range newEntries {
		key := plannedEntryContentKey(e)
		newByContent[key] = e
	}

	for contentKey, oldEntry := range oldByContent {
		if _, exists := newByContent[contentKey]; !exists {
	...
	for _, newEntry := range newEntries {
		contentKey := plannedEntryContentKey(newEntry)
		oldEntry, existsInOld := oldByContent[contentKey]
	...
	plan.Entry = flattenEntries(ctx, remoteState, &resp.Diagnostics)
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}
