...
func (r *Resource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	nested := NestedModel{
		...
		AutoLoadbalance:     plan.AutoLoadbalance,
		BetweenBytesTimeout: plan.BetweenBytesTimeout,
		ConnectTimeout:      plan.ConnectTimeout,
		ErrorThreshold:      plan.ErrorThreshold,
		FirstByteTimeout:    plan.FirstByteTimeout,
		HealthCheck:         plan.HealthCheck,
		KeepaliveTime:       plan.KeepaliveTime,
		MaxConn:             plan.MaxConn,
		MaxLifetime:         plan.MaxLifetime,
		MaxTLSVersion:       plan.MaxTLSVersion,
		MaxUse:              plan.MaxUse,
		MinTLSVersion:       plan.MinTLSVersion,
		OverrideHost:        plan.OverrideHost,
		PreferIPv6:          plan.PreferIPv6,
		RequestCondition:    plan.RequestCondition,
		ShareKey:            plan.ShareKey,
		Shield:              plan.Shield,
		SSLCACert:           plan.SSLCACert,
		SSLCertHostname:     plan.SSLCertHostname,
		SSLCheckCert:        plan.SSLCheckCert,
		SSLCiphers:          plan.SSLCiphers,
		SSLClientCert:       plan.SSLClientCert,
		SSLClientKey:        plan.SSLClientKey,
		SSLSNIHostname:      plan.SSLSNIHostname,
		UseSSL:              plan.UseSSL,
		Weight:              plan.Weight,
	}
	opts := BuildCreateInput(plan.Service.ValueString(), int(plan.Version.ValueInt64()), nested)
...
func (r *Resource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	stateNested := NestedModel{
		Name:                state.Name,
		Address:             state.Address,
		Port:                state.Port,
		Comment:             state.Comment,
		AutoLoadbalance:     state.AutoLoadbalance,
		BetweenBytesTimeout: state.BetweenBytesTimeout,
		ConnectTimeout:      state.ConnectTimeout,
		ErrorThreshold:      state.ErrorThreshold,
		FirstByteTimeout:    state.FirstByteTimeout,
		HealthCheck:         state.HealthCheck,
		KeepaliveTime:       state.KeepaliveTime,
		MaxConn:             state.MaxConn,
		MaxLifetime:         state.MaxLifetime,
		MaxTLSVersion:       state.MaxTLSVersion,
		MaxUse:              state.MaxUse,
		MinTLSVersion:       state.MinTLSVersion,
		OverrideHost:        state.OverrideHost,
		PreferIPv6:          state.PreferIPv6,
		RequestCondition:    state.RequestCondition,
		ShareKey:            state.ShareKey,
		Shield:              state.Shield,
		SSLCACert:           state.SSLCACert,
		SSLCertHostname:     state.SSLCertHostname,
		SSLCheckCert:        state.SSLCheckCert,
		SSLCiphers:          state.SSLCiphers,
		SSLClientCert:       state.SSLClientCert,
		SSLClientKey:        state.SSLClientKey,
		SSLSNIHostname:      state.SSLSNIHostname,
		UseSSL:              state.UseSSL,
		Weight:              state.Weight,
	}
