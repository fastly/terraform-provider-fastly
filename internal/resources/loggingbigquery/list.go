...
func (l *ListResource) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		Description: "List all BigQuery logging endpoints across all Fastly CDN and Compute services at their active version, or latest version when no active version exists.",
...
func (l *ListResource) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	tflog.Debug(ctx, "Listing Fastly BigQuery logging endpoints")
	...
	stream.Results = func(push func(list.ListResult) bool) {
		...
		for _, svc := range services {
			...
			loggers, err := l.client.ListBigQueries(ctx, &fastly.ListBigQueriesInput{
				ServiceID:      serviceID,
				ServiceVersion: version,
			})
			if err != nil {
				tflog.Warn(ctx, "Error listing BigQuery logging endpoints for service", map[string]any{
					"service_id": serviceID,
					"error":      err.Error(),
				})
				continue
			...
			for _, bq := range loggers {
				if bq == nil || bq.Name == nil {
					continue
				}
...
func setResourceAttrs(ctx context.Context, result *list.ListResult, bq *fastly.BigQuery, serviceID string, version int) diag.Diagnostics {
	...
	model := Model{
		NestedModel: FlattenToNestedModel(bq),
