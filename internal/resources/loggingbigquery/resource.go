...
func (r *Resource) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_service_logging_bigquery"
}

func (r *Resource) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Fastly service BigQuery logging endpoint resource. Writes directly to the specified writable service version.",
...
func (r *Resource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	tflog.Debug(ctx, "Creating Fastly BigQuery logging endpoint", map[string]any{
		"service_id": plan.Service.ValueString(),
		"version":    plan.Version.ValueInt64(),
		"name":       service.StringValue(plan.Name),
	})

	if err := validation.EnsureServiceTypeSupported(ctx, r.providerData.ServiceTypeChecker, plan.Service.ValueString(), "fastly_service_logging_bigquery", service.TypeVCL, service.TypeCompute); err != nil {
	...
	bq, err := r.providerData.Client.CreateBigQuery(ctx, input)
	if err != nil {
		resp.Diagnostics.AddError("Error creating BigQuery logging endpoint", err.Error())
		return
	}
...
func (r *Resource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	tflog.Debug(ctx, "Reading Fastly BigQuery logging endpoint from API", map[string]any{
		"service_id": state.Service.ValueString(),
		"version":    state.Version.ValueInt64(),
		"name":       state.Name.ValueString(),
	})

	bq, err := r.providerData.Client.GetBigQuery(ctx, &fastly.GetBigQueryInput{
	...
	if err != nil {
		if errors.IsNotFound(err) {
			tflog.Warn(ctx, "BigQuery logging endpoint not found, removing from state", map[string]any{
				"service_id": state.Service.ValueString(),
				"name":       state.Name.ValueString(),
			})
		...
		resp.Diagnostics.AddError("Error reading BigQuery logging endpoint", err.Error())
		return
	}
...
func (r *Resource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting Fastly BigQuery logging endpoint", map[string]any{
		"service_id": state.Service.ValueString(),
		"version":    state.Version.ValueInt64(),
		"name":       state.Name.ValueString(),
	})

	if err := validation.EnsureServiceTypeSupported(ctx, r.providerData.ServiceTypeChecker, state.Service.ValueString(), "fastly_service_logging_bigquery", service.TypeVCL, service.TypeCompute); err != nil {
		resp.Diagnostics.AddError("Unsupported service type", err.Error())
		return
	}
	...
	err := r.providerData.Client.DeleteBigQuery(ctx, &fastly.DeleteBigQueryInput{
		ServiceID:      state.Service.ValueString(),
		ServiceVersion: int(state.Version.ValueInt64()),
		Name:           state.Name.ValueString(),
	})
	if err != nil {
		...
		resp.Diagnostics.AddError("Error deleting BigQuery logging endpoint", err.Error())
	}
}
...
func (r *Resource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError(
			...
			"Expected import ID in format: service_id/version/name\n"+
				"For example: service123/3/my-bigquery-logger\n\n"+
				"Error: "+err.Error(),
		)
		return
	}

	tflog.Debug(ctx, "Importing BigQuery logging endpoint", map[string]any{
	...
	if err != nil {
		resp.Diagnostics.AddError("Error importing BigQuery logging endpoint", err.Error())
		return
	}
