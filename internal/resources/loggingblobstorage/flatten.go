...
func FlattenToNestedModel(s *fastly.BlobStorage) NestedModel {
	m := NestedModel{}
	...
	m.Container = types.StringValue(fastly.ToValue(s.Container))
	m.Authentication = NewAuthenticationObject(
		service.StringPointerOrDefault(s.AccountName, ""),
		service.StringPointerOrDefault(s.SASToken, ""),
	)
	...
	m.Format = service.StringPointerOrDefault(s.Format, constants.LoggingBlobStorageDefaultFormat)
	...
	m.FileMaxBytes = service.Int64PointerOrDefault(s.FileMaxBytes, DefaultFileMaxBytes)
...
func ResetVCLOnlyToDefaults(m *NestedModel) {
	m.Format = types.StringValue(constants.LoggingBlobStorageDefaultFormat)
...
func FlattenToComputeNestedModel(s *fastly.BlobStorage) ComputeNestedModel {
...
func flatten(ctx context.Context, s *fastly.BlobStorage, m *Model) {
	if s == nil {
		tflog.Warn(ctx, "flatten called with nil Blob Storage logging endpoint")
		return
	}
	...
	tflog.Debug(ctx, "Flattened Blob Storage logging endpoint state", map[string]any{
		"id":      id,
