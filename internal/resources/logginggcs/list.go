...
func (l *ListResource) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		Description: "List all GCS logging endpoints across all Fastly CDN and Compute services at their active version, or latest version when no active version exists.",
...
func (l *ListResource) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	tflog.Debug(ctx, "Listing Fastly GCS logging endpoints")
	...
	stream.Results = func(push func(list.ListResult) bool) {
		...
		for _, svc := range services {
			...
			loggers, err := l.client.ListGCSs(ctx, &fastly.ListGCSsInput{
				ServiceID:      serviceID,
				ServiceVersion: version,
			})
			if err != nil {
				tflog.Warn(ctx, "Error listing GCS logging endpoints for service", map[string]any{
					"service_id": serviceID,
					"error":      err.Error(),
				})
				continue
			...
			for _, g := range loggers {
				...
				result.DisplayName = service.ToGeneratedResourceName(fastly.ToValue(svc.Name), serviceID, *g.Name)

				if req.IncludeResource {
					result.Diagnostics.Append(setResourceAttrs(ctx, &result, g, serviceID, version, fastly.ToValue(svc.Type))...)
				}
...
// accept.
func setResourceAttrs(ctx context.Context, result *list.ListResult, g *fastly.GCS, serviceID string, version int, serviceType string) diag.Diagnostics {
	var diags diag.Diagnostics

	id := serviceID + "-" + fmt.Sprintf("%d", version) + "-" + fastly.ToValue(g.Name)

	model := Model{
		NestedModel: FlattenToNestedModel(g),
