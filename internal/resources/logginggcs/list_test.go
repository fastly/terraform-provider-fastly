...
// normalization gap as TestResetVCLOnlyToDefaults (see logginggcs_test.go),
...
func TestSetResourceAttrs_computeResetsVCLOnlyFields(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantFormat:        types.StringValue(constants.LoggingGCSDefaultFormat),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			diags := setResourceAttrs(ctx, &result, g, "service-id", 1, tt.serviceType)
