...
func TestAuthenticationEitherOr(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "nothing configured, shared account_name env var set",
			accountName: types.StringNull(),
			email:       types.StringNull(),
			secretKey:   types.StringNull(),
		...
		{
			name:                 "nothing configured, GCS's own account_name env var set",
			accountName:          types.StringNull(),
			email:                types.StringNull(),
			secretKey:            types.StringNull(),
			envDeprecatedAccount: "gcs-svc-account",
			wantError:            false,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			t.Setenv("FASTLY_GCS_EMAIL", tt.envEmail)
			t.Setenv("FASTLY_GCS_SECRET_KEY", tt.envSecret)
...
func TestGzipLevelCodecConflict(t *testing.T) {
	tests := []struct {
		...
		{"gzip_level unset", types.Int64Null(), types.StringNull(), false},
		{"gzip_level only", types.Int64Value(5), types.StringNull(), false},
		{"gzip_level with blank codec", types.Int64Value(5), types.StringValue(""), false},
		{"gzip_level with codec set", types.Int64Value(5), types.StringValue("gzip"), true},
		{"gzip_level unknown", types.Int64Unknown(), types.StringValue("gzip"), false},
	}
...
func TestNotTrimmed(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.value, func(t *testing.T) {
			...
			notTrimmed{}.ValidateString(context.Background(),
