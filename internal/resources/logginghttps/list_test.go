...
// normalization gap as TestResetVCLOnlyToDefaults (see logginghttps_test.go),
...
func TestSetResourceAttrs_computeResetsVCLOnlyFields(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantFormat:        types.StringValue(constants.LoggingHTTPSDefaultFormat),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			diags := setResourceAttrs(ctx, &result, h, "service-id", 1, tt.serviceType)
