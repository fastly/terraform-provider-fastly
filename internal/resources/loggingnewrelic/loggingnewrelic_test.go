...
func defaultNestedModel() NestedModel {
	return NestedModel{
		...
		Format:            types.StringValue(constants.LoggingNewRelicDefaultFormat),
...
func fullNestedModel() NestedModel {
	...
	m.Authentication = NewAuthenticationObject(types.StringValue("newrelic-api-key"))
...
func TestBuildCreateInput(t *testing.T) {
	tests := []struct {
		...
		version   int
		model     NestedModel
		validate  func(t *testing.T, input *fastly.CreateNewRelicInput)
	}{
		{
			name:      "minimal model",
			...
			validate: func(t *testing.T, input *fastly.CreateNewRelicInput) {
				...
				assert.Equal(t, "newrelic-api-key", *input.Token)
				assert.Equal(t, "US", *input.Region)
				assert.Equal(t, "none", *input.ProcessingRegion)
				assert.Equal(t, constants.LoggingNewRelicDefaultFormat, *input.Format)
...
func TestResetVCLOnlyToDefaults(t *testing.T) {
	...
	m := FlattenToNestedModel(&fastly.NewRelic{
		Name:              new("test-newrelic"),
		Token:             new("newrelic-api-key"),
		Region:            new("US"),
	...
	assert.Equal(t, constants.LoggingNewRelicDefaultFormat, m.Format.ValueString())
	...
	assert.Equal(t, "newrelic-api-key", m.Token().ValueString())
...
// `logging_newrelic { format = ... }` inside fastly_service_compute_auto fail
...
func TestAuthenticationAttribute(t *testing.T) {
	...
	assert.True(t, auth.Required, "authentication is Required: there is no FASTLY_NEWRELIC_* env var to default it from")
	...
	assert.True(t, token.Sensitive, "the New Relic API key must never be rendered in plan output")

	assert.Len(t, auth.Attributes, 1, "authentication holds only token for New Relic")
}
...
func TestTokenAccessor(t *testing.T) {
	assert.Equal(t, "newrelic-api-key", minimalNestedModel().Token().ValueString())
...
// values the Fastly API documents for New Relic: only US and EU, unlike
// Datadog's larger region set.
