...
func (r *Resource) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Fastly service New Relic logging endpoint resource. Writes directly to the specified writable service version.",
...
func (r *Resource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	tflog.Debug(ctx, "Creating Fastly New Relic logging endpoint", map[string]any{
		"service_id": plan.Service.ValueString(),
		"version":    plan.Version.ValueInt64(),
		"name":       service.StringValue(plan.Name),
	})

	if err := validation.EnsureServiceTypeSupported(ctx, r.providerData.ServiceTypeChecker, plan.Service.ValueString(), "fastly_service_logging_newrelic", service.TypeVCL, service.TypeCompute); err != nil {
	...
	n, err := r.providerData.Client.CreateNewRelic(ctx, input)
	if err != nil {
		resp.Diagnostics.AddError("Error creating New Relic logging endpoint", err.Error())
		return
	}
...
func (r *Resource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	tflog.Debug(ctx, "Reading Fastly New Relic logging endpoint from API", map[string]any{
		"service_id": state.Service.ValueString(),
		"version":    state.Version.ValueInt64(),
		"name":       state.Name.ValueString(),
	})

	n, err := r.providerData.Client.GetNewRelic(ctx, &fastly.GetNewRelicInput{
	...
	if err != nil {
		if errors.IsNotFound(err) {
			tflog.Warn(ctx, "New Relic logging endpoint not found, removing from state", map[string]any{
				"service_id": state.Service.ValueString(),
				"name":       state.Name.ValueString(),
			})
		...
		resp.Diagnostics.AddError("Error reading New Relic logging endpoint", err.Error())
		return
	}
...
func (r *Resource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	tflog.Debug(ctx, "Updating Fastly New Relic logging endpoint", map[string]any{
		"service_id": opts.ServiceID,
		"version":    opts.ServiceVersion,
		"name":       opts.Name,
	})

	n, err := r.providerData.Client.UpdateNewRelic(ctx, opts)
	if err != nil {
		resp.Diagnostics.AddError("Error updating New Relic logging endpoint", err.Error())
		return
	}
...
func (r *Resource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting Fastly New Relic logging endpoint", map[string]any{
		"service_id": state.Service.ValueString(),
		"version":    state.Version.ValueInt64(),
		"name":       state.Name.ValueString(),
	})

	if err := validation.EnsureServiceTypeSupported(ctx, r.providerData.ServiceTypeChecker, state.Service.ValueString(), "fastly_service_logging_newrelic", service.TypeVCL, service.TypeCompute); err != nil {
		resp.Diagnostics.AddError("Unsupported service type", err.Error())
		return
	}
	...
	err := r.providerData.Client.DeleteNewRelic(ctx, &fastly.DeleteNewRelicInput{
		ServiceID:      state.Service.ValueString(),
		ServiceVersion: int(state.Version.ValueInt64()),
		Name:           state.Name.ValueString(),
	})
	if err != nil {
		...
		resp.Diagnostics.AddError("Error deleting New Relic logging endpoint", err.Error())
	}
}
...
func (r *Resource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	...
	tflog.Debug(ctx, "Importing New Relic logging endpoint", map[string]any{
	...
	if err != nil {
		resp.Diagnostics.AddError("Error importing New Relic logging endpoint", err.Error())
		return
	}
