...
func (l *ListResource) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		Description: "List all New Relic OTLP logging endpoints across all Fastly CDN and Compute services at their active version, or latest version when no active version exists.",
...
func (l *ListResource) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	tflog.Debug(ctx, "Listing Fastly New Relic OTLP logging endpoints")
	...
	stream.Results = func(push func(list.ListResult) bool) {
		...
		for _, svc := range services {
			...
			loggers, err := l.client.ListNewRelicOTLP(ctx, &fastly.ListNewRelicOTLPInput{
				ServiceID:      serviceID,
				ServiceVersion: version,
			})
			if err != nil {
				tflog.Warn(ctx, "Error listing New Relic OTLP logging endpoints for service", map[string]any{
					"service_id": serviceID,
					"error":      err.Error(),
				})
				continue
