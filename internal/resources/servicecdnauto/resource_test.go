...
func TestAccFastlyServiceCDNAuto_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy:             testAccCheckServiceCDNAutoDestroy,
		Steps: []resource.TestStep{
			{
				Config: configBasic(serviceName, domainName),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckServiceCDNAutoExists("fastly_service_cdn_auto.test"),
...
func testAccCheckServiceCDNAutoDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "fastly_service_cdn_auto" {
			continue
		}
...
func testAccCheckServiceCDNAutoExists(resourceName string) resource.TestCheckFunc {
...
func configBasic(serviceName, domainName string) string {
	return testfixtures.BuildConfig(
		...
		"internal/resources/servicecdnauto/test_fixtures/configs/basic.tf",
