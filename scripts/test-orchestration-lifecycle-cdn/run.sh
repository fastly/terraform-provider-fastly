...
TEST_DIR="$REPO_ROOT/test-orchestration-$$"
TF_CONFIG_DIR="$SCRIPT_DIR"

# Test configuration
TEST_SERVICE_1_NAME="tf-test-orch-svc1-$$"
TEST_SERVICE_2_NAME="tf-test-orch-svc2-$$"
SERVICE_1_ID=""
SERVICE_2_ID=""
...
test_resource_updates() {
    ...
    sed -i '' 's/comment       = "Test service 1 for orchestration"/comment       = "Updated test service 1"/' main.tf
...
test_resource_destruction() {
    ...
    log_info "Removing version-locked resources from Terraform state..."
...
# Run all tests
main() {
    log_info "Starting orchestration lifecycle test"
    ...
    echo ""
    log_success "All orchestration lifecycle tests passed!"
}
