...
resource "fastly_service_compute" "service_1" {
  ...
  comment       = "Test compute service 1 for orchestration"
  force_destroy = true
}
...
resource "fastly_service_compute" "service_2" {
  ...
  comment       = "Test compute service 2 for orchestration"
  force_destroy = true
}
...
action "fastly_compute_package_upload" "service_1_upload" {
  config {
    service_id   = fastly_service_compute.service_1.id
    version      = var.service_1_version
...
action "fastly_compute_package_upload" "service_2_upload" {
  config {
    service_id   = fastly_service_compute.service_2.id
    version      = var.service_2_version
    package_path = var.package_path
