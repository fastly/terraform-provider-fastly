...
EOF
    ...
    # Upload package to version 4 and activate it
    log_info "Uploading package to version 4..."
    if terraform apply -invoke=action.fastly_compute_package_upload.service_1_upload -auto-approve; then
        log_success "Package uploaded to version 4"
    else
        log_error "Failed to upload package to version 4"
        return 1
    fi
...
test_resource_destruction() {
    ...
    terraform state rm 'fastly_service_domain.service_1_new_domain[0]' || true
    terraform state rm 'fastly_service_backend.service_1_new_backend[0]' || true
